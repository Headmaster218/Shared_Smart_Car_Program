...
void direction1(u8 go_mode,float s)
{
	float s0;
	//走的模式
	switch(go_mode)
	{
	  ...
	  //Drive_Motor(0.1,0,0);  
			if(s<s0/6)
			{
				Drive_Motor(0.06,-0.024,0); 
			}
			else if((s>s0/6)&&(s<s0*5/6))
			{
				Drive_Motor(0.25,-0.1,0); 
			}
			else if(s> s0 * 5/6 &&s0 - s > 0.5)
			{
				Drive_Motor(0.06,-0.024,0); 
			}
			else if(s0 - s <= 0.5)
		...
		case 3:
			...
			TIM_SetCompare2(TIM12, 1100);
			TIM_SetCompare3(TIM8, 1700);
			TIM_SetCompare1(TIM12, 550);
...
{ 
    ...
    //TIM12->CCR1=1500;
	  //TIM12->CCR2=1500;	
	  //TIM_SetCompare1(TIM12, 1500);  
	  //TIM_SetCompare2(TIM12, 1500);
    //TIM8_SERVO_Init(9999,168-1);	
	  //TIM_SetCompare1(TIM12, 1500);
	 while(1){
		 /*if( Time_count>2200 && Time_count<2400)	  
		 {TIM_SetCompare2(TIM12, 1200);//PWM越大，夹爪张得越窄
      TIM_SetCompare4(TIM8, 2000);//PWM越小（500,2500）--（0,180）
		  TIM_SetCompare3(TIM8, 1000);
		 }
		 else if(Time_count>2400)
     TIM_SetCompare2(TIM12, 1600);*/ 
      // TIM_SetCompare2(TIM12, 1700);		 
		   //TIM_SetCompare1(TIM12, 550);//PWM小，顺时针（500,1500）
		   //TIM_SetCompare4(TIM8, 1500);//PWM越小（500,2500）--（0,180）
		   //TIM_SetCompare3(TIM8, 1700);
		   //TIM_SetCompare2(TIM8, 800);
		   //TIM_SetCompare1(TIM8, 500);
		   //TIM_SetCompare2(TIM8, 800);
		  /*if( Time_count>1800 && Time_count<2600)
		   {
			  TIM_SetCompare2(TIM8, 800);
		    TIM_SetCompare2(TIM12, 1100);
				TIM_SetCompare3(TIM8, 1700);
		    ...
		    TIM_SetCompare4(TIM8, 1500);
			 ...
			 else if(Time_count>2600 && Time_count<2800)
			 {
			  TIM_SetCompare2(TIM12, 1700);
			 }
			 else if(Time_count>2800 && Time_count<3800)
			 {
			   TIM_SetCompare1(TIM12, 900);
				 direction(2,s,200);
			 }
			 else if(Time_count>3800 && Time_count<4000)
			 {
			  
				 TIM_SetCompare2(TIM12, 1100);
			 }
			  else if(Time_count>4000 && Time_count<4500)
			 {
			  
				 TIM_SetCompare1(TIM12, 550);
			 }
			 else if(Time_count>4500 && Time_count<5500)
			 { 
				 TIM_SetCompare2(TIM12, 1700);
				 TIM_SetCompare1(TIM12, 900);
				 direction(1,s,217);
			 }
			 else if(Time_count>5500 )
			 {
			   Drive_Motor(0,0,0);
			 }*/
			/* else if(Time_count>5700 && Time_count<5900)
			 {
			  TIM_SetCompare1(TIM12, 550); 
			 }
			  else if(Time_count>5900 && Time_count<6100)
			 {
			  
			 }
			 else if(Time_count>4500)
				 TIM_SetCompare2(TIM12, 1700);*/
       //TIM_SetCompare2(TIM12, 1500);
    ...
    //TIM8->CCR3=1500;
	  //TIM8->CCR4=1500;
	   ...
	   //TIM12->CCR1=1500;
			...
			//时间计数，30秒后不再需要
			//if(200 - s >=1)
		 if(Time_count<6000) Time_count++;
		 Get_Velocity_Form_Encoder(); ;
		 s=s+float_abs(MOTOR_A.Encoder);
		 /*s=s+MOTOR_A.Encoder;
     if(s<0)	OLED_ShowString(15,10,"-"),
			        OLED_ShowNumber(20,10,-s,5,12);
           ...
           //TIM_SetCompare1(TIM12, 1750);
              ...
              direction(4,s,580);*/								
				 ...
				 //{
					/*if(Time_count<3000)
					 Drive_Motor(0, 0.4, 0);
					else if(1500<=Time_count && Time_count<1600)
					 Drive_Motor(0, 0, 0);
					else if(1600<=Time_count && Time_count<3000)
					 Drive_Motor(0, 0.4, 0);
...
返回  值：无
**************************************************************************/
void direction(int go_mode,float s,float s0)
{		
 	Drive_Motor(Move_X, Move_Y, Move_Z); 
	switch(go_mode)
	{ 
		case 1:
	   if(MOTOR_A.Encoder==0 && MOTOR_B.Encoder==0 && MOTOR_C.Encoder==0 && MOTOR_D.Encoder==0)
		 Move_X=0.2,Move_Y=0,Move_Z=0;	
	   //Drive_Motor(Move_X, Move_Y, Move_Z);   
	   if( (s<=s0 && s0-s<=4) || s>s0)
		 {
     ...
     break; 		 
	  case 2:
	   ...
	   if((s<=s0 && s0-s<=4) || s>s0)
		 ...
		 case 5:
			 if(MOTOR_A.Encoder==0 && MOTOR_B.Encoder==0 && MOTOR_C.Encoder==0 && MOTOR_D.Encoder==0)	 
			 Move_X=0.25,Move_Y=-0.1,Move_Z=0;
			 if((s<=s0 && s0-s<=3) || s>s0)
		 {
			Move_X=0,Move_Y=0,Move_Z=0;
		 }
